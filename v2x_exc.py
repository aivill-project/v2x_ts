...
def plot_confusion_matrix(cm, classes, normalize=False, title='Confusion matrix', cmap=plt.cm.Blues):
    ...
    plt.savefig(f"output/confusion_matrix_{args.class_name}_{LOG_START_TIME}_{int(normalize)}.png")
...
def get_preds_csv(learner, classes):
    ...
    for idx in range(len(test_targets)):
        ...
        test_precision = round(skm.precision_score(test_targets[:idx+1], test_preds[:idx+1], average = "micro"), 4)
        test_recall = round(skm.recall_score(test_targets[:idx+1], test_preds[:idx+1], average = "micro"), 4)
        test_f1 = round(skm.f1_score(test_targets[:idx+1], test_preds[:idx+1], average = "micro"), 4)
    ...
    plot_confusion_matrix(test_targets, test_preds, classes=list(classes.values()), normalize=True, title='Normalized confusion matrix')
    plot_confusion_matrix(test_targets, test_preds, classes=list(classes.values()), normalize=False, title='Confusion matrix')
